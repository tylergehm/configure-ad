...
[Download User-Generator.ps1](https://github.com/tylergehm/configure-ad/raw/main/Create-ADUsers.ps1)
 ...
 <img width="1901" height="1075" alt="image" src="https://github.com/user-attachments/assets/32fce871-2981-4303-a101-d133701d0035" /> </p>

 This step will begin in the Domain Controller VM. Click the Start menu and search for Powershell. Right click on Powershell ISE and select "Run as Administrator". </p>

PowerShell ISE (Integrated Scripting Environment) is a built-in Windows application that provides a graphical editor and debugging tool for writing, testing, and running PowerShell scripts. You must right-click → Run as administrator when executing PowerShell scripts that modify Active Directory because AD requires elevated privileges. Only domain administrators (or accounts with delegated rights) can create users, OUs, or change domain objects. Without elevation, it uses your current login’s permissions, which may be limited. User Account Control prevents non-admin processes from altering protected system areas (including AD). </p>

Running as admin launches PowerShell with full domain admin rights, allowing the script to successfully write to Active Directory. </p>
If you skip this step, you’ll get Access Denied errors. </p>

<img width="1710" height="882" alt="image" src="https://github.com/user-attachments/assets/79a7ba9b-0340-4537-b774-68342dbdfb17" /> </p>

Once inside PowershelL ISE, click on the button for "New Script" </p>

<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/2a019790-93a0-49ea-aed0-4ca6a2b07a74" /> </p>

Once a new script has been created, paste the User-Generator Powershell script, and save the script on the Desktop. </p>

<img width="1703" height="873" alt="image" src="https://github.com/user-attachments/assets/94e1032b-e6ff-42d1-b71c-529bf899a784" /> </p>

Next, click on the button to "Run Script". </p>

<img width="1712" height="884" alt="image" src="https://github.com/user-attachments/assets/2ef2c4ee-0808-4620-81cc-0c76f8d3e05a" /> </p>

The script is successfully running, as a list of users being generated is now shown in Powershell ISE. </p>

<img width="942" height="662" alt="image" src="https://github.com/user-attachments/assets/78edc84f-3d0b-4d51-9f55-580731db43e1" /> </p>

Verifying that Users have been created in the "EMPLOYEES" OU of the mydomain.com domain. </p>

<img width="549" height="664" alt="image" src="https://github.com/user-attachments/assets/8e0c6154-b7dd-4505-a01a-8cd3d84bb35e" /> </p>

Using Remote Desktop Connection, the users generated by the script will be tested by attempting to log in as one of the generated users.</p>
For this test, the random user "bixil.dehi" was chosen. The RDC is logging into the Client-1 VM and note how the username must contain the domain (mydomain.com\bixil.dehi). </p>

<img width="1916" height="965" alt="image" src="https://github.com/user-attachments/assets/d319d852-39be-458f-b859-59cb34d8fc28" /> </p>

Once logged into Client-1 VM as the user bixil.dehi, identity can be verified by examining command prompt. In the C:\Users folder, all the users that have logged into this virtual machine during the project have storage folders created.

<h2>Conclusion</h2> </p>
This Azure lab successfully demonstrates the deployment and configuration of an on-premises-style Active Directory environment using Windows Server 2025 as a domain controller and Windows 11 as a client, all within a secure virtual network. By setting static IPs, configuring DNS, promoting the server to manage a new forest (mydomain.com), joining the client to the domain, enabling remote access for non-admin users, and automating user creation with PowerShell, the project replicates real-world enterprise identity management in the cloud. It provides hands-on experience with core AD concepts—authentication, policy enforcement, organizational units, and automation—proving that Azure can reliably host traditional directory services while offering scalability, accessibility, and centralized control for modern IT administration.
